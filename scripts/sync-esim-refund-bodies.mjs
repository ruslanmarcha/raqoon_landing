...
const BODIES = path.join(__dirname, 'esim-refund-bodies')
...
for (const [code, file] of MAP) {
  ...
  if (!data.legal?.esim?.refund) throw new Error(`No legal.esim.refund in ${file}`)
  const body = fs.readFileSync(source, 'utf8').replace(/\r\n/g, '\n')
  data.legal.esim.refund.body = body
